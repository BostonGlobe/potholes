...
write.csv(csv, str_c('output/bestDayForDistrict', unique(district$pwd_district), '.csv'), row.names = FALSE)
...
***
#### Show me clusters of 15 or more potholes fixed on the same day for all districts in 2014.
```{r, results='asis', out.width='910px', fig.width=10, fig.height=7, warning=FALSE, message=FALSE, dpi=50, dev='jpeg'}
data <- potholes %>%
  filter(
    LATITUDE!=42.3594, LONGITUDE!=-71.0587,
...
ggmap(map) +
  ...
  ggtitle(str_c('Clusters of 15 or more daily pothole closures by district, in 2014')) +
  scale_size_area(max_size=10)
```
